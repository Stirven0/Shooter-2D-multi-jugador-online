...
- **Tile Engine**: Formato Tiled JSON (.tmj). TileMap se envía 1 vez vía MAP_DATA, no en cada GameState. Caché en cliente para render. Tiles sólidos → Obstacles con horizontal run merging. Sprite sheet warehouse.png para texturas, fallback a TileColors.
